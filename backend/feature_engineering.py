...
def build_features(
    ...
    # =====================================================
    # 3. LAG FEATURES & EWMA MOMENTUM
    # =====================================================
    lags = [1, 2, 3, 4, 24, 168, 336, 720]
    ...
    for lag in lags:
        ...
        df[f"lag_{lag}"] = raw_lag.fillna(expanding_mean).infer_objects(copy=False)
        df[f"region_lag_{lag}"] = df[f"lag_{lag}"]

    # Add the EWMA Demand Momentum you trained on!
    df["avg_pickups"] = base_pickup.shift(1).ewm(alpha=0.4).mean().round()
    ...
    holiday_dates = pd.to_datetime(list(us_hols.keys()))
    ...
    # =====================================================
    # 6. WEATHER FEATURES (THE CRITICAL FIX)
    # =====================================================
    weather_cols = [
        "temperature_2m", "apparent_temperature", "precipitation", "snowfall",
        "snow_depth", "windspeed_10m", "cloudcover", "relativehumidity_2m",
        "is_raining", "is_snowing", "is_heavy_weather", "is_freezing",
        "is_storm_jonas", "temp_bin", "weather_condition"
    ]

    df["hour_floor"] = pd.to_datetime(df.index).floor("h").tz_localize(None)
    weather_df = weather_df.copy()
    weather_df.index = pd.to_datetime(weather_df.index).tz_localize(None)

    # CRITICAL: Save the index before merging so it isn't destroyed
    ...
    # CRITICAL: Restore the index immediately
    ...
    # =====================================================
    # 8. TARGET ROW EXTRACTION
    # =====================================================
    # Because we restored the index, this will now successfully pull the real data!
    ...
    categorical_cols = ["holiday_name", "temp_bin", "weather_condition"]

    for col in target_row.columns:
        if col in categorical_cols:
            target_row[col] = target_row[col].astype(str).replace("nan", "unknown").fillna("unknown").astype("category")
        else:
            target_row[col] = pd.to_numeric(target_row[col], errors="coerce")

    # Fill any remaining NaNs in numeric columns
    numeric_cols = target_row.select_dtypes(include=["number"]).columns
    target_row[numeric_cols] = target_row[numeric_cols].fillna(0)
